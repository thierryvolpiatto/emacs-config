...
(defun ledger-reverse-date-from-regexp (regexp)
  (let ((ledger-file (getenv "LEDGER_FILE")))
    (cl-assert ledger-file nil "No ledger file found, check your ENV var LEDGER_FILE")
...
(defvar ledger/associations '(("PRELEVEMENT A LA SOURCE REVENUS" . ":impot:prelevement_source")
                              ...
                              ("REMB IMPOT REVENUS" . ":impot:impot_revenu")
                              ("MOTIF: CARAC" . ":carac")
                              ("MOTIF: DECLIC" . ":codevi")
                              ("GITHUB SPONSORS" . ":thierry:github")
                              ...
                              ("J LOUIS DAVID" . ":soins:coiffeur_rachel")
                              ("LAURENT" . ":soins:coiffeur_rachel")
                              ("DELOMPRE CURBILL" . ":soins:estheticienne")
                              ("BIO ET BIEN ETRE" . ":alimentation:la_vie_claire")
                              ("AUCHAN" . ":alimentation:auchan")
                              ("LIDL" . ":alimentation:lidl")
                              ("MARCEL & FILS" . ":alimentation:bio:marcel")
                              ("INTERMARCHE" . ":alimentation:intermarche")
                              ("CARREFOUR MARKET" . ":alimentation:carrefour")
                              ("BOUYGUES" . ":telephone:bouygues")
                              ("COTISATION MENSUELLE SOBRIO DUO" . ":banque:socgen")
                              ("OPTION TRANQUILLITE" . ":banque:socgen")
                              ("COTIS OPTION CRYPTO DYNAMIQUE" . ":banque:socgen")
                              ("PHARM" . ":santé:pharmacie")
                              ("BLOC SESSION" . ":sport:bs")
                              ))
    ...
    (save-excursion
      (cl-loop for (regexp . tag) in ledger/associations
               do (progn
                    (goto-char (point-min))
                    (while (re-search-forward regexp nil t)
                      (when (re-search-forward "\\(Expenses\\|Income\\)\\(:?[^ \n]*\\)\\( *€? *[0-9.]*\\)$" nil t)
                        (unless (string= tag (match-string 2))
                          (replace-match tag t nil nil 2)))))))))
